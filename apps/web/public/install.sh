...
link_npm_binary_to_wrapper() {
    ...
    # binary has #!/usr/bin/env node, which fails in a fresh
    # interactive shell when mise's shims dir (where node lives) isn't
    # on the operator's PATH. The wrapper prepends mise/shims to PATH
    # before exec'ing so `env node` resolves cleanly.
    _mise_data="${MISE_DATA_DIR:-$HOME/.local/share/mise}"
    cat > "$WRAPPER" <<WRAP_EOF
#!/bin/sh
# Auto-generated by the Infernet Protocol installer.
# Ensures node is resolvable before exec'ing the npm-installed CLI.
...
WRAP_EOF
    chmod +x "$WRAPPER"
    ok "wrote wrapper $WRAPPER → $NPM_INFERNET (PATH-fixed)"
}
