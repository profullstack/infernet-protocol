...
export class InferenceCoordinator {
  ...
  async registerWorker(ws, data) {
    ...
    try {
      // Verify the node exists in Supabase
      ...
      // Update node_roles in Supabase
      try {
        // Check if role record exists
        const { data: roleRecords, error: roleError } = await this.supabase
          .from('node_roles')
          .select('*')
          .eq('node', nodeId);
   ...
   *
   * @param {string} jobId - ID of the job in Supabase
  ...
  async startJob(jobId, jobConfig) {
    try {
      ...
      const { data: distributedJob, error: createError } = await this.supabase
  ...
  async finalizeJob(jobId) {
    ...
    try {
      ...
      const { data: distributedJobs, error: listError } = await this.supabase
        .from('distributed_jobs')
        .select('*')
        .eq('job', jobId);
      ...
      if (distributedJobs && distributedJobs.length > 0) {
        const { error: updateDistError } = await this.supabase
          .from('distributed_jobs')
          .update({ status: 'completed' })
          .eq('id', distributedJobs[0].id)
          .select()
          .single();
  ...
  async updateWorkerStatus(workerId, statusData) {
    ...
    try {
      const { data: roleRecords, error: listError } = await this.supabase
        .from('node_roles')
        .select('*')
        .eq('node', workerId);
      if (listError) throw listError;
      ...
      console.error(`Error updating worker status in Supabase for ${workerId}:`, error);
    }
  }
